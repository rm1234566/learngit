<%@ page language="java" import="java.util.*" import="DAOs.FriendDAO" import="DAOs.UserDAO" import="java.text.SimpleDateFormat" pageEncoding="UTF-8" contentType="text/html;charset=UTF-8"%>
<%
...
<html>
  ...
  <body style="background:#d1faff">
    ...
    <div style="position:absolute;left:15%;top:10%;">
      <%for(int i = 0;i < nrOfFriends;i++){ int u_id = fdao.getFriendByIndex(i).getU_id_2();%>
        <form method="post" action="deleteFriend.do">
          <table cellspacing="20" style="width: 1530px; ">
          <tr style="height: 50px; "><td>好友: <%=udao.getUserNameByUserId(u_id)%></td>
          <td><input type="text" name="u_id_2" style="display:none;" value=<%=u_id%>></td>
          </tr>
          <tr><td><input type="submit" name="submit" value="删除好友"></td></tr>
          </table>
        </form>
      <%}%>
    </div>
